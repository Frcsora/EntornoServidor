...
function guardarMensaje($PDO, $usuario, $mensaje){
    $stmt =$PDO->prepare("INSERT INTO mensajes (usuario, mensaje) VALUES (:usuario, :mensaje)");
    $stmt->bindParam(':usuario', $usuario, PDO::PARAM_INT);
    $stmt->bindParam(':mensaje', $mensaje, PDO::PARAM_STR);
    $stmt->execute();
}
function selectMensajes($PDO, $ultimo){
    $stmt = $PDO->prepare("SELECT usuarios.username, mensajes.* FROM mensajes
                        JOIN usuarios ON usuarios.id = mensajes.usuario
                        WHERE mensajes.id > :ultimo;");
    $stmt->bindParam(':ultimo', $ultimo, PDO::PARAM_INT);
    $stmt->execute();
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    return $result;
}
function truncateTable($PDO){
    $stmt = $PDO->prepare("DELETE FROM mensajes");
    $stmt->execute();
}